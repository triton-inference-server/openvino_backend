...
class TestLoadedModel:
    def test_inference(self, model, triton_server):
        ...
        for shape in config["shape"]:
            ...
            assert output.shape in config["expected_shape"], "Invalid shape returned from the server, expected {config[\"expected_shape\"]} got {output.shape}"
    ...
    def test_model_ready(self, model, triton_server):
        ...
        assert response.ready == True, f"Invalid status reported from the server, expected True got {response.ready}"
